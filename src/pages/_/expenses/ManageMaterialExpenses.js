...
const ManageMaterialExpenses = () => {
    ...
    const query = useQuery(['all-raw-material-expenses', debouncedSearch, state.globals.shop], () =>
        get('/raw-material-expenses', { page: 1, limit: 5, sort: { name: 1 }, search: debouncedSearch })
    );

    const deleteMutation = useMutation((id) => del(`/raw-material-expenses/id/${id}`), {
        onSuccess: async () => {
            alert.showAlert({ color: 'success', heading: 'Material expense deleted successfully' });
            await queryClient.invalidateQueries('all-raw-material-expenses');
        },
        onError: (err) => {
            alert.showAlert({ color: 'danger', heading: 'Unable to delete material expense', err });
        },
    });
    ...
    const handleEdit = useCallback(
        (shop) => {
            batch(() => {
                dispatch(setMaterialExpensesData(shop));
                dispatch(setMaterialExpensesVisibility(true));
            });
        },
        [dispatch]
    );
            ...
            <Card>
                <CardBody>
                    <Row className="form-group">
                        <Col xl={12}>
                            <Button color="info" onClick={() => dispatch(setMaterialExpensesVisibility(true))}>
                                Add Material Expense
                            </Button>
                            <Input
                                className="float-right tw-max-w-[400px]"
                                placeholder="Search Material Expenses"
                                value={search}
                        ...
                        <Col xl={12}>
                            <p className="card-title-desc">
                                {!query.isLoading && !query.isError
                                    ? `Showing ${query.data?.docs.length} material expenses`
                                    : '...'}
                            </p>
                        </Col>
                                    ...
                                    <tbody>
                                        ...
                                        <When condition={query.isError}>
                                            <EmptyTableRow columnCount={5}>
                                                <>
                                                    Unable to load material expenses
                                        ...
                                        <When condition={query.isSuccess}>
                                            <If condition={query.data?.docs.length}>
                                                <Then>
                                                    {query.data?.docs.map((e, index) => (
                                                        <tr key={`customer-${index}`}>
                                                            ...
                                                            <td className="tw-align-middle">{e.product_bought}</td>
                                                ...
                                                <Else>
                                                    <EmptyTableRow columnCount={5}>
                                                        No material expense created yet
                                                    </EmptyTableRow>
                                                </Else>
                                            </If>
                                        </When>
...
export default ManageMaterialExpenses;
