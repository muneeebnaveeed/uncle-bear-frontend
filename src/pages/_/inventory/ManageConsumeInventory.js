...
import { setAddInventoryData, setAddInventoryVisibility, setConsumeInventoryData, setConsumeInventoryVisibility } from '../../../store/actions';
import useAlert from '../../../components/Common/useAlert';
import EmptyTableRow from '../../../components/Common/EmptyTableRow';
import FormatNumber from '../../../components/Common/FormatNumber';

const ManageConsumeInventory = ({ startDate, endDate }) => {
    ...
    const query = useQuery(['all-consume-inventory-transactions', startDate, endDate, debouncedSearch, state.globals.shop], () =>
        get('/inventories/transactions/consume', { page: 1, limit: 5, search: debouncedSearch, startDate, endDate })
    );
    ...
    const deleteMutation = useMutation((id) => del(`/inventories/id/${id}`), {
        onSuccess: async () => {
            ...
            await queryClient.invalidateQueries('all-consume-inventory-transactions');
        },
        onError: (err) => {
                    ...
                    <Row className="form-group">
                        <Col xl={12}>
                            <Button color="info" onClick={() => batch(() => {
                                dispatch(setAddInventoryData({ type: 'consume' }))
                            ...
                            <Input
                                ...
                                placeholder="Search Consume Inventory Transactions"
                                value={search}
                        ...
                        <Col xl={12}>
                            <p className="card-title-desc">
                                {!query.isLoading && !query.isError
                                    ? `Showing ${query.data?.docs.length} consume inventory transactions`
                                    : '...'}
                            </p>
                        </Col>
                            ...
                            <ListGroup>
                                ...
                                <When condition={query.isError}>
                                    <ListGroupItem className="tw-flex tw-items-center tw-justify-between">
                                        Unable to load consume inventory transactions
                                ...
                                <When condition={query.isSuccess}>
                                    <If condition={query.data?.docs.length}>
                                        ...
                                        <Else>
                                            <ListGroupItem className="tw-flex tw-items-center tw-justify-between tw-items-center">
                                                No consume inventory transaction found
                                            </ListGroupItem>
                                        </Else>
                                    </If>
                                </When>
...
export default ManageConsumeInventory;
